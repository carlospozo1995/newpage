<?php
    ...
    $searchData = $template_vars['search'];
    $searchedProducts = Models_Resultado::showSearchData($searchData);

    if (!empty($searchedProducts)) {

        $products = $searchedProducts['products'];
        $amountProd = $searchedProducts['dataSummary']['dataPriceAmount'][0]['amount_products'];
        $price_min = $searchedProducts['dataSummary']['dataPriceAmount'][0]['price_min'];
        $price_max = $searchedProducts['dataSummary']['dataPriceAmount'][0]['price_max'];
        $amount_brands = $searchedProducts['dataSummary']['dataBrand'];
...
?>
                                ...
                                <div class="breadcrumb-nav breadcrumb-nav-color--black">
                                    <nav aria-label="breadcrumb">
                                        <ul class="navigation-page">
                                            ...
                                            <li class="active"><?= count($searchData) < 2 ? strtoupper($searchData[0]) : strtoupper($searchData[0]).'-'.strtoupper($searchData[1]); ?></li>
                                        </ul>
                                    </nav>
                                </div>
                            </div>
            ...
            <div class="shop-section">
                <div class="container">
                    <input type="hidden" id="data-store" value="<?php
                        if (is_string($data)) {
                            echo Utils::encryptStore($data);
                        }
                        if (is_array($data)) {
                           echo count($searchData) < 2 ? $searchData[0] : $searchData[0].'-'.$searchData[1];
                        }
                    ?>">
                                        ...
                                        <div class="sort-box d-flex justify-content-between align-items-md-center align-items-start flex-md-row flex-column"
                                            ...
                                            <div class="sort-tablist d-flex align-items-center">
                                                ...
                                                <div class="page-amount ml-2">
                                                    <span><?= $amountProd; ?> Productos</span>
                                                </div> <!-- End Page Amount -->
